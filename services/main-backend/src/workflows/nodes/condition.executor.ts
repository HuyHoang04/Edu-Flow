...
export class ConditionNodeExecutor implements NodeExecutor {
    async execute(node: any, context: any, execution: any, workflow?: any): Promise<NodeExecutionResult> {
        ...
        try {
            if (typeof condition === 'string') {
                ...
                if (parts.length === 3 && !parts[0].includes('.')) {
                    ...
                    const leftValue = context[leftKey] !== undefined ? context[leftKey] : Number(leftKey);
                    const rightValue = !isNaN(Number(rightVal)) ? Number(rightVal) : rightVal;
                    ...
                    switch (operator) {
                        ...
                        case '==': result = leftValue == rightValue; break;
                        case '!=': result = leftValue != rightValue; break;
                        default: console.warn(`[ConditionNode] Unknown operator: ${operator}`);
                    }
                } else {
                    // Fallback to evaluating as JS expression
                    console.log('[ConditionNode] Evaluating as JS expression:', condition);
        ...
        if (workflow && workflow.edges) {
            ...
            const edges = workflow.edges.filter((e: any) =>
                e.source === node.id &&
                (e.sourceHandle === targetHandle || e.sourceHandle === (result ? trueLabel : falseLabel))
            );
            nextNodes = edges.map((e: any) => e.target);
            console.log(`[ConditionNode] Next nodes for ${targetHandle}: ${nextNodes.join(', ')}`);
        }
