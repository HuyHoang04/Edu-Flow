...
export class CreateExamNodeExecutor implements NodeExecutor {
    ...
    async execute(node: any, context: any, execution: any, workflow?: any): Promise<any> {
        ...
        if (questions.length > 0) {
            questions = questions.sort(() => 0.5 - Math.random()).slice(0, Number(questionCount));
        }
        ...
        const exam = await this.examsService.create({
            ...
            deadline: new Date(Date.now() + (Number(duration) || 60) * 60000 * 24 * 7), // Default deadline 1 week from now
            createdBy: context.userId || node.data.createdBy // Ensure createdBy is present
        });

        console.log(`[CreateExamNode] Created exam: ${exam.id} with ${examQuestions.length} questions`);
